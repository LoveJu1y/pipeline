module CPU(
	...
	wire [5  -1:0] Write_register;//which reg to write
	...
	RegisterFile register_file1(
		...
		.clk            (clk                ),
		.RegWrite       (RegWrite           ), 
		.Read_register1 (Instruction[25:21] ), //rs
		.Read_register2 (Instruction[20:16] ), //rt
		.Write_register (Write_register     ),
		.Write_data     (Databus3           ), //输入寄存器堆的那条线
		.Read_data1     (Databus1           ), //上边的寄存器读取的结果
		.Read_data2     (Databus2           )//下面寄存器读取的结果
	);

	// Extend
	wire [32 -1:0] Ext_out;//extop后的数据
	...
	assign ALU_in1 = ALUSrc1? {27'h00000, Instruction[10:6]}: Databus1;//【10:6】是移位的量，对于一些移位的指令，所以，alusrc1就是这条数据上是否移位的标志。
	assign ALU_in2 = ALUSrc2? LU_out: Databus2;//立即数的通道或者下面的寄存器
	...
	wire           Memory_Read      ;
	wire           Memory_Write     ;
	wire [32 -1:0] MemBus_Read_Data ;//要传出去的数据

	DataMemory data_memory1(
		.reset      (reset              ), 
		.clk        (clk                ), 
		.Address    (MemBus_Address     ), //无论读写，就是一个address
		.Write_data (MemBus_Write_Data  ), 
		.Read_data  (Memory_Read_Data   ), //读出来的数据
		.MemRead    (Memory_Read        ), //这两个都是一位的控制信号
